...
### Per-Row Content
- Map thumbnail (56×56pt, rounded corners). A static image captured once at ride end (#177), never a live map per row, which would be a performance issue: the recorded track in `brMapTravelPath` over a muted map with points of interest hidden, stored on the ride in light and dark. Place and road names can't be switched off in MapKit's standard style, so a few may show. A ride recorded before #177, or with no GPS track, shows a placeholder.
