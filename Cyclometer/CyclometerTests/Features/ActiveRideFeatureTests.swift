...
// MARK: - Alert escalation

/// #135: AlertLevel (L0–L3) derivation and haptic/audio dispatch. Proves the
/// mechanism works end-to-end; the exhaustive boundary/transition matrix is #138's
/// scope. `RadarTarget.threatLevel` is pinned to `.allClear` throughout — that field
/// only drives `isCalibrationSuspended` (a separate concern, its own suite above),
/// and these tests care about `relativeVelocityMPS` only.
@MainActor
@Suite("ActiveRideFeature — alert escalation")
struct ActiveRideFeatureAlertEscalationTests {
    ...
    func advisoryToCaution() async {
        ...
        await store.send(.radarTargetsUpdated([Self.vehicle(mps: 2)])) {
            $0.radarTargets = [Self.vehicle(mps: 2)]
            $0.activeAlertLevel = .advisory
            $0.lastAlertDispatchAt = [.advisory: testDate]
        }

        let secondDate = testDate.addingTimeInterval(ActiveRideFeature.alertReTriggerInterval + 1)
    ...
    func cautionToDangerRepeatsThenClears() async {
        ...
        let dangerDate = testDate.addingTimeInterval(ActiveRideFeature.alertReTriggerInterval + 1)
        ...
        let clearDate = dangerDate.addingTimeInterval(ActiveRideFeature.alertReTriggerInterval + 1)
    ...
    func dangerToCautionNoReAlert() async {
        ...
        let downgradeDate = dangerDate.addingTimeInterval(ActiveRideFeature.alertReTriggerInterval + 1)
        store.dependencies.date.now = downgradeDate
        let backToCaution = [Self.vehicle(mps: 20 / 3.6)]
    ...
    func cautionToClear() async {
        ...
        let clearDate = testDate.addingTimeInterval(ActiveRideFeature.alertReTriggerInterval + 1)
        store.dependencies.date.now = clearDate
        await store.send(.radarTargetsUpdated([])) {
            $0.radarTargets = []
            $0.activeAlertLevel = .clear
    ...
    func guardBlocksSameLevelReTriggerWithinThreeSeconds() async {
        ...
        let advisoryTargets = [Self.vehicle(mps: 2)]
        await store.send(.radarTargetsUpdated(advisoryTargets)) {
            $0.radarTargets = advisoryTargets
            $0.activeAlertLevel = .advisory
    ...
    @Test("Disconnect-triggered advisory shares the same guard as vehicle-based dispatch")
    func disconnectSharesTheReTriggerGuard() async {
        let clock = TestClock()
        let counts = LockIsolated(Counts())
        let store = makeStore(clock: clock, counts: counts)
        ...
        await store.receive(\.radarReconnectTimedOut) {
            $0.isRadarPaired = false
            $0.radarTargets = []
            // activeAlertLevel is already .advisory; guard blocks the haptic since
            // less than 3s have passed since the first timeout.
        }
        #expect(counts.value.hapticAdvisory == 1)  // unchanged

        // Advance the mocked wall clock past the guard, then time out a third time.
        let laterDate = testDate.addingTimeInterval(4)
        ...
        await store.receive(\.radarReconnectTimedOut) {
            $0.isRadarPaired = false
            $0.radarTargets = []
            $0.lastAlertDispatchAt = [.advisory: laterDate]
        }
        #expect(counts.value.hapticAdvisory == 2)
    }
